...
function checkuser(username)
{
    ...
    const checkuser1 = `SELECT * FROM users WHERE username = '${username}'`;

    alert('checkuser1'+ checkuser1);
    
    client.query(checkuser1, (error, results) => {
        //console.log(error, response);
        alert('Results: '+results.rowCount);
        if (error){
            alert(error);
        }
        alert('Results: '+results.rowCount);
